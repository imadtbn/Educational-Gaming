 ...
 */
import { Check, Eraser, Image, RefreshCw, Sparkles, Trash2, Volume2 } from "lucide-react";
...
export default function Writing() {
  ...
  const { completeActivity, deleteArtwork, progress, saveArtwork } = useProgress();
  ...
  const saveCurrentArtwork = () => { const source = canvasRef.current; if (!source) return; const thumbnail = document.createElement("canvas"); thumbnail.width = 360; thumbnail.height = 184; const context = thumbnail.getContext("2d"); if (!context) return; context.fillStyle = "#fffdf5"; context.fillRect(0, 0, thumbnail.width, thumbnail.height); context.drawImage(source, 0, 0, thumbnail.width, thumbnail.height); saveArtwork({ symbol: track.symbol, label: track.label, image: thumbnail.toDataURL("image/jpeg", .76) }); };
  const celebrate = () => { if (!hasInk || complete) return; setComplete(true); saveCurrentArtwork(); playTone("success"); completeActivity({ activityId: `writing-${track.symbol}`, stars: 1, badge: "writing-trail", title: "أثر قلم رائع", message: `أكملت تدريب كتابة ${track.label} وحُفظ أثره في سجل رحلتك.` }); };
  const next = () => { setTrackIndex((value) => (value + 1) % tracks.length); playTone("tap"); };

  return <SiteLayout><div className="writing-page"><section className="writing-hero"><div><Link href="/" className="back-link">← كل المسارات</Link><p className="eyebrow"><span>✦</span> لوح أثر القلم</p><h1>اكتب، جرّب…<br /><em>واترك أثرك.</em></h1><p>اتبع الدليل الشفاف بالقلم أو الإصبع. هنا لا توجد إجابة خاطئة؛ كل محاولة تقرّبك من شكل الحرف والرقم.</p><div className="inner-route writing-route"><span>استمع</span><i /><b>✎</b><i /><span>اكتب واحتفل</span></div></div><div className="writing-hero-art" aria-hidden="true"><div className="paper-pencil"><i /><b /></div><span className="pencil-ticket">أثر اليوم</span><i>✦</i><i>✧</i></div></section><section className="writing-station"><aside className="track-picker" aria-label="اختيار تدريب الكتابة"><p>اختر أثر اليوم</p>{tracks.map((item, index) => <button type="button" key={item.symbol} className={index === trackIndex ? `is-active ${item.color}` : ""} onClick={() => setTrackIndex(index)}><b>{item.symbol}</b><span>{item.label}</span></button>)}</aside><div className="writing-board"><div className="board-station-label"><span>⚑</span> محطة 03 · أثر القلم <i>✦</i></div><div className="board-topline"><div><span>{track.type}</span><h2>{track.label}</h2></div><button type="button" className="board-sound" onClick={speak}><Volume2 size={18} /> استمع للنطق</button></div><div className="canvas-frame"><canvas ref={canvasRef} width="900" height="460" onPointerDown={start} onPointerMove={move} onPointerUp={() => setDrawing(false)} onPointerLeave={() => setDrawing(false)} aria-label={`مساحة كتابة ${track.label}`} />{complete && <div className="ink-celebration" aria-hidden="true"><i>✦</i><i>★</i><i>✧</i><strong>أحسنت! حُفظ أثر قلمك.</strong></div>}</div><div className="writing-actions"><button type="button" onClick={clear}><Eraser size={17} /> امسح وحاول</button><button type="button" className="finish-writing" onClick={celebrate} disabled={!hasInk}><Check size={17} /> أنهيت التدريب</button><button type="button" onClick={next}><RefreshCw size={17} /> أثر جديد</button></div><div className="board-route-stamp" aria-hidden="true"><span>استمع</span><i /><b>✎</b><i /><span>نجمة</span></div></div></section><section className="artwork-log" aria-label="سجل آثار القلم"><div className="artwork-log-heading"><div><p>سجل رحلتي</p><h2>آثار قلمي المحفوظة</h2><span>تُحفظ على هذا الجهاز فقط، ويمكنك حذف أي أثر لاحقًا.</span></div><span className="artwork-count"><Image size={16} /> {progress.artworks.length} / 8 آثار</span></div>{progress.artworks.length ? <div className="artwork-grid">{progress.artworks.map((artwork) => <article className="artwork-card" key={artwork.id}><img src={artwork.image} alt={`رسم محفوظ لتدريب ${artwork.label}`} /><div><strong>{artwork.symbol}</strong><span>{artwork.label}</span><small>{new Intl.DateTimeFormat("ar", { day: "numeric", month: "short" }).format(new Date(artwork.createdAt))}</small></div><button type="button" aria-label={`حذف رسم ${artwork.label}`} onClick={() => deleteArtwork(artwork.id)}><Trash2 size={15} /></button></article>)}</div> : <div className="artwork-empty"><Image size={27} /><p>بعد إنهاء أي تدريب، سيظهر أثر قلمك هنا لتشاهده لاحقًا.</p></div>}</section><div className="writing-ad"><AdSlot label="مساحة إعلانية — بين آثار القلم" /></div></div></SiteLayout>;
}
